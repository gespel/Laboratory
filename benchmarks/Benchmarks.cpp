TEST_CASE ("Boot performance")
{
    ...
    {
        std::vector<Catch::Benchmark::storage_for<LaboratoryAudioProcessor>> storage (size_t (meter.runs()));
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        std::vector<Catch::Benchmark::destructable_object<LaboratoryAudioProcessor>> storage (size_t (meter.runs()));
        for (auto& s : storage)
            s.construct();
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        ...
        meter.measure ([&] (int /* i */) {
            auto editor = plugin.createEditorIfNecessary();
